...
class GridNet(nn.Module):
    def __init__(self, in_channels, in_channels1, in_channels2, in_channels3, out_channels, attention=False):
        ...
        if attention:
            self.attention_layer0 = nn.Sequential(nn.Conv2d(2, 32, 1), 
            nn.Sigmoid())
            self.attention_layer1 = nn.Sequential(nn.Conv2d(2, 64, 1), 
            nn.Sigmoid())
            self.attention_layer2 = nn.Sequential(nn.Conv2d(2, 96, 1), 
            nn.Sigmoid())


    def forward(self, x, x1, x2, x3, sigma=None):
        if sigma is not None:
            sigma_u = F.interpolate(sigma, x2.shape[2:], mode='bilinear')
            sigma_uu = F.interpolate(sigma, x1.shape[2:], mode='bilinear')
            att0 = self.attention_layer0(sigma_uu)
            att1 = self.attention_layer1(sigma_u)
            att2 = self.attention_layer2(sigma)
