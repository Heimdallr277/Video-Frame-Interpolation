...
def load_dataset(dataset_str, data_root, batch_size, val_batch_size, num_workers, img_fmt='png'):

    if dataset_str == 'vimeo90k':
    ...
    val_loader = get_loader('val', data_root, val_batch_size, shuffle=False, num_workers=num_workers)
...
def build_input(images, imgpaths, is_training=True, include_edge=False, device=torch.device('cuda')):
    ...
    im0, im1 = images[0].to(device), images[2].to(device)
...
def init_meters():
    ...
    ternary_losses = AverageMeter()
    reconstru_losses = AverageMeter()
    losses = total_losses, gt_losses, ternary_losses, reconstru_losses
    psnrs = AverageMeter()
    ssims = AverageMeter()
    lpips = AverageMeter()
...
def calc_metrics(im_pred, im_gt, mask=None):
    q_im_pred = quantize(im_pred.data)
    q_im_gt = quantize(im_gt.data)
    if mask is not None:
...
def log_tensorboard(writer, losses, psnr, ssim, lpips, lr, timestep, mode='train'):
    ...
    writer.add_scalar('Loss/%s/%s' % (mode, 'total'), losses[0].avg, timestep)
    writer.add_scalar('Loss/%s/%s' % (mode, 'gt'), losses[1].avg, timestep)
    writer.add_scalar('Loss/%s/%s' % (mode, 'perceptual'), losses[2].avg, timestep)
    writer.add_scalar('Loss/%s/%s' % (mode, 'reconstruc'), losses[3].avg, timestep)
...
def save_batch_images(output, imgpath, save_dir, alpha=0.5):
    ...
    q_im_output = [quantize(o.data) for o in output]
