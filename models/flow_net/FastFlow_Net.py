...
class Decoder(nn.Module):
    def __init__(self, in_channels, groups, out_channels=2+1):
        ...
        self.conv7 = nn.Conv2d(32, out_channels, 3, 1, 1)
...
class FastFlowNet(nn.Module):
    def __init__(self, groups=3):
        ...
        self.rconv1 = convrelu(16, 32, 3, 1)
        ...
        self.up3 = deconv(2+1, 2+1)
        self.up4 = deconv(2+1, 2+1)
        self.up5 = deconv(2+1, 2+1)
        self.up6 = deconv(2+1, 2+1)
        
        self.decoder2 = Decoder(87+1, groups)
        self.decoder3 = Decoder(87+1, groups)
        self.decoder4 = Decoder(87+1, groups)
        self.decoder5 = Decoder(87+1, groups)
        self.decoder6 = Decoder(87+1, groups)
    ...
    def forward(self, x):
        ...
        # flow7_up = torch.zeros(f16.size(0), 2+1, f16.size(2), f16.size(3)).to(f15)
        ...
        # f25_w = backwarp(f25, flow6_up[:,0:2,:,:]*0.625)
        flow6_up = torch.zeros(f15.size(0), 2+1, f15.size(2), f15.size(3)).to(f15)
        cv5 = torch.index_select(self.corr(f15, f25), dim=1, index=self.index.to(f15).long())
        r15 = self.rconv5(f15)
        cat5 = torch.cat([cv5, r15, flow6_up], 1)
        flow5 = self.decoder5(cat5) + flow6_up

        flow5_up = self.up5(flow5)
        f24_w = backwarp(f24, flow5_up[:,0:2,:,:]*1.25)
        ...
        f23_w = backwarp(f23, flow4_up[:,0:2,:,:]*2.5)
        ...
        f22_w = backwarp(f22, flow3_up[:,0:2,:,:]*5.0)
        ...
        flow = flow2[:,0:2,:,:]
        sigma = torch.tanh(self.conv(flow2[:,2:,:,:]))
        ...
        # H, W = img1.shape[2:]
        # Hf, Wf = flow2.shape[2:]
        # ratio_x = float(W) / float(Wf)
        # ratio_y = float(H) / float(Hf)
        # flow = F.interpolate(flow2, size=[H, W], mode='bilinear')
        # flow[:, 0, :, :] *= ratio_x
        # flow[:, 1, :, :] *= ratio_y
